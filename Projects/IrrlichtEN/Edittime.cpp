...
int WINAPI DLLExport CreateObject(mv _far *mV, fpLevObj loPtr, LPEDATA edPtr)
{
	...
	if ( IS_COMPATIBLE(mV) )
	{
		//mainEngineEditor = edPtr;
		// Set default object settings
		edPtr->common.swidth = 640;
		edPtr->common.sheight = 480;
...
{
#ifndef RUN_ONLY
	edPtr->common.swidth = cx;
	edPtr->common.sheight = cy;
#endif // !defined(RUN_ONLY)
	return TRUE;	// OK
}
...
{
#ifndef RUN_ONLY
	rc->right = rc->left + edPtr->common.swidth;
	rc->bottom = rc->top + edPtr->common.sheight;
#endif // !defined(RUN_ONLY)
	return;
}
...
void WINAPI DLLExport EditorDisplay(mv _far *mV, fpObjInfo oiPtr, fpLevObj loPtr, LPEDATA edPtr, RECT FAR *rc)
{
	...
	if ( ps != NULL )		// Do the following if this surface exists
	{
		...
		is.Fill(255, 0, 255);											// and remap palette if necessary
